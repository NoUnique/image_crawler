...
function getLinks() {
    var links = document.querySelectorAll('a.rg_l');
    return Array.prototype.map.call(links, function(e) {
        var url = unescape(e.getAttribute('href')).match("imgurl\=(.*)\&imgrefurl");
        return url[1];
    });
}

function countDivs() {
  var divs = document.querySelectorAll('div.rg_di');
  return divs.length;
}

casper.start('https://www.google.co.kr/imghp', function() {
  this.fill('form[action="https://www.google.co.kr/search"]', { q: query }, true);
});
...
casper.then(function() {
  // #smb : id of 'View more results' button. Maybe 'Search More Button'??
  if (searchmore) {
    this.click('#smb');
    this.wait(wait_time);
  }
...
casper.then(function() {
  ...
  casper.then(function() {
    ...
    if (count > count_prev) {
      ...
      // .rg_di .rg_bx .rg_el .ivg-i : classes of image div
      // data-ri : index attirubte of images in google image search page
      var last  = this.getElementAttribute('div.rg_di[data-ri="'+(count-1)+'"]', 'href');
      if (last == null) {
        this.goto('LOOP_AFTER_SMB');
      }
